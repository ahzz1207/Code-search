...
class CodeSearcher:
	...
	def train(self, model):
		...
		for i in range(self.conf.reload, nb_epoch):
			...
			# outputs = [layer.output for layer in self.code_repr_model.layers]
			# model.getOutput([chunk_padded_methnames, chunk_padded_apiseqs, chunk_padded_tokens, chunk_padded_good_descs,
			# 	 chunk_padded_bad_descs])
